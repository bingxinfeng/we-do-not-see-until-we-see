...
void main() {
    ...
    //alpha mask:
    //float mask = (texture(offsetTexture, texCoord).g+texture(offsetTexture, texCoord).r+texture(offsetTexture, texCoord).b)/3;
    float mask = texture(offsetTexture, texCoord).a;
